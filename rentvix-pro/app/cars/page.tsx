...
export default function ModernCarManagement() {
  ...
  const handleUpdateCar = async () => {
    try {
      ...
      setCars(
        cars.map((car) => (car.id === editingCar.id ? res.data.data : car))
      );
      setEditingCar(null);
  ...
  const totalValue = cars.reduce(
    (sum, car) => sum + (car["nilai-beli"] || 0),
    0
  );
  const totalDepreciation = cars.reduce(
    (sum, car) => sum + (car["penyusutan-aset"] || 0),
    0
  );
      ...
      <div className="max-w-7xl mx-auto p-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Kelola semua aset kendaraan dalam armada rental Anda
          </p>
        </div>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {editingCar
                      ? "Ubah informasi mobil yang dipilih"
                      : "Masukkan informasi lengkap mobil baru"}
                  </DialogDescription>
                </DialogHeader>
            ...
            {filteredCars.map((car) => (
              ...
              >
                <div className="relative">
                  ...
                  <Badge className="absolute top-3 right-3 bg-green-100 text-green-700 hover:bg-green-100">
                    Aktif
                  </Badge>
                </div>
                <CardHeader>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      {new Date(car["tanggal-pembelian"]).toLocaleDateString(
                        "id-ID"
                      )}
                    </div>
                  </div>
                  <div className="mb-4 space-y-1">
                    <p className="text-sm text-gray-600">
                      <span className="font-medium">No. Rangka:</span>{" "}
                      {car["no-rangka"]}
                    </p>
                    <p className="text-sm text-gray-600">
                      <span className="font-medium">No. Mesin:</span>{" "}
                      {car["no-mesin"]}
                    </p>
                    <p className="text-sm text-gray-600">
